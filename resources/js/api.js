...
const createCharts = (data) => {
    ...
    const averagePrice = closePrices.reduce((total, price) => total + price, 0) / closePrices.length;
    ...
    const donchianDatasets = datasetRanges.map(range => {
        const { max, min } = findMinMaxClosePrice(closePrices, range.range[0], range.range[1]);
    ...
    const allDonchianDatasets = donchianLowerBoundDatasets.concat(donchianUpperBoundDatasets);
